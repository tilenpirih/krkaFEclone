...
onMounted(async () => {
  ...
  if (query.template) {
    ...
    if (templateData.value.selectedCompetition) {
      matches.value = await kzsStore.getMatches({ seasonId: templateData.value.selectedSeason, competitionId: templateData.value.selectedCompetition.id, statuses: 'CREATED,SCHEDULED' })
    }
...
function changeMatch() {
  if (!templateData.value.selectedMatch)
    return
  texts.value = []
  const team1Index = images.value.findIndex(image => image.originalFile.name === 'Ekipa1.png')
  if (team1Index !== -1) {
    images.value.splice(team1Index, 1)
    images.value.splice(team1Index, 0, { originalFile: { name: 'Ekipa1.png', url: `${runtimeConfig.public.API_URL}/getKzsImage?image_uuid=${templateData.value.selectedMatch.firstTeamLogoUuid}` }, croppedFile: { url: null, coordinates: { width: 80000, height: 80000, left: 96, top: 0 } }, width: 300, height: 300, xMargin: 125, yMargin: 950, darken: 100, opacity: 100, fit: 'cover' })
  }
  else {
    images.value.push({ originalFile: { name: 'Ekipa1.png', url: `${runtimeConfig.public.API_URL}/getKzsImage?image_uuid=${templateData.value.selectedMatch.firstTeamLogoUuid}` }, croppedFile: { url: null, coordinates: { width: 80000, height: 80000, left: 96, top: 0 } }, width: 300, height: 300, xMargin: 125, yMargin: 950, darken: 100, opacity: 100, fit: 'cover' })
  }
  const team2Index = images.value.findIndex(image => image.originalFile.name === 'Ekipa2.png')
  if (team2Index !== -1) {
    images.value.splice(team2Index, 1)
    images.value.splice(team2Index, 0, { originalFile: { name: 'Ekipa2.png', url: `${runtimeConfig.public.API_URL}/getKzsImage?image_uuid=${templateData.value.selectedMatch.secondTeamLogoUuid}` }, croppedFile: { url: null, coordinates: { width: 80000, height: 80000, left: 96, top: 0 } }, width: 300, height: 300, xMargin: 655, yMargin: 950, darken: 100, opacity: 100, fit: 'cover' })
  }
  else {
  ...
  texts.value.push({ text: `VS`, xMargin: 0, yMargin: 980, size: 125, color: '#000000', bold: true, italic: false, underline: false, justify: 'center', opacity: 100, xSize: 100, font: 'Tomarik Brush' })
  ...
  texts.value.push({ text: dayjs(templateData.value.selectedMatch.dateTime).format('D. M. YYYY H:mm'), xMargin: 0, yMargin: 230, size: 80, color: '#fff', bold: true, italic: false, underline: false, justify: 'center', opacity: 100, xSize: 100, font: 'Poppins' })
  texts.value.push({ text: `${templateData.value.selectedMatch.sportHallName}`, xMargin: 0, yMargin: 360, size: 60, color: '#fff', bold: true, italic: false, underline: false, justify: 'center', opacity: 100, xSize: 100, font: 'Poppins' })
...
async function saveTheme() {
  ...
  if (route.query.template) {
    if (typeof route.query.template !== 'string')
      return
    await templateStore.updateTemplate(route.query.template, templateName.value, texts.value, images.value, resolutionStore.selectedWidth, resolutionStore.selectedHeight, templateData.value, 'napoved_tekme', base64Image)
    templateStore.templates = []
    router.push('/')
  }
  else {
    await templateStore.addTemplate(templateName.value, texts.value, images.value, resolutionStore.selectedWidth, resolutionStore.selectedHeight, templateData.value, 'napoved_tekme', base64Image)
    templateStore.templates = []
    router.push('/')
  }
  saveModal.value = false
